<template>
    <div class="ma-4">
        ...
        <v-data-table :headers="headers" :items="itemsGastos" :search="search">
            <template v-slot:item.actions="{ item }">
                <v-row class="justify-center align-center">
                    <v-col class="col-6">
                        <v-tooltip>
                            <template v-slot:activator="{ props }">
                                <v-icon size="x-large" class="mr-2" @click="editarGastos(item)" color="success"
                                    v-bind="props">
                                    mdi mdi-pencil
                                </v-icon>
                            </template>
                    ...
                    <v-col class="col-6">
                        <v-tooltip>
                            <template v-slot:activator="{ props }">
                                <v-icon size="x-large" class="mr-2" @click="eliminarGastos(item)" color="danger"
                                    v-bind="props">
                                    mdi mdi-trash-can
                                </v-icon>
                            </template>
    ...
    <v-dialog v-model="dialogoGastos" max-width="60%" transition="dialog-top-transition" persistent>
        <v-card class="pa-6 rounded-lg elevation-12">
            ...
            <!-- Formulario -->
            <v-card-text>
                <v-form ref="refsGastos">
                    <v-row class="mb-4 justify-space-between" dense>
                        <v-col cols="6" md="6" class="justify-center">
                            <v-text-field v-model="data.concepto" label="Concepto" variant="outlined" rounded
                                :rules="[...requiredRule]" />
                        </v-col>
                        <v-col cols="6" md="6">
                        ...
                        <v-menu v-model="data.menu" transition="scale-transition" :close-on-content-click="false">
                            <template v-slot:activator="{ props }">
                                <v-text-field v-bind="props" rounded variant="outlined" class="custom-text-field"
                                    label="Fecha de gasto" :rules="[...requiredRule]"
                                    v-model="computedFechaGasto"></v-text-field>
                            </template>
                            <v-date-picker v-model="data.fecha"
                    ...
                    <v-row class="mb-4 justify-space-between">
                        <v-col cols="6" md="6" class="justify-center">
                            <v-select rounded variant="outlined" v-model="data.responsable"
                                label="Responsable" :items="itemsResponsable" item-value="id" item-title="name" />
                        </v-col>
                    </v-row>
                    ...
                    <v-row justify="center">
                        <v-col cols="12" md="6" class="d-flex justify-center gap-4">
                            <v-btn v-if="banderaDialogo == 1" color="success" @click.prevent="guardarGastos()"
                                rounded>
                                <v-icon start icon="mdi-content-save" />
                                Guardar
                            </v-btn>
                            <v-btn v-if="banderaDialogo == 2" color="success" @click.prevent="editarSaveGastos()"
                                rounded>
                                <v-icon start icon="mdi-content-save" />
                                Actualizar
                            </v-btn>
...
export default {
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { title: 'RESPONSABLE', value: 'nombreResponsable' },
    ...
    methods: {
        getData() {
            axios.get('/colaborador/getGastos')
                .then(response => {
                    this.itemsGastos = response.data.gastos;
                    this.itemsCategoria = response.data.categorias.original.catGastos;
                    this.itemsResponsable = response.data.responsable;
                })
        },

        openDialog() {
            this.overlay = true;
            this.titleDialogo = "Agregar Gastos";
            this.banderaDialogo = 1;
            this.dialogoGastos = true;
            this.overlay = false;
        },

        closeDialog() {
            this.overlay = true;
            this.dialogoGastos = false;
            this.overlay = false;
            // this.idItem = null;
            if (this.$refs.refsGastos) this.$refs.refsGastos.reset();
        },

        async guardarGastos() {

            const resul = await this.$refs.refsGastos.validate();

            if (resul.valid) {
                this.overlay = true;
                axios.post('/colaborador/guardarGastos', {
                    data: this.data,
                }).then(res => {
                    this.overlay = false;
        ...
        editarGastos(item) {
            ...
            this.overlay = true;
            this.idItem = item.id;
            this.data.concepto = item.concepto;
            ...
            this.data.categoria = item.gastosCategoriaId;
            this.data.responsable = item.responsableId;
            this.titleDialogo = 'Editar Gasto';
            this.banderaDialogo = 2;
            this.dialogoGastos = true;
            this.overlay = false;
        },

        async editarSaveGastos() {
            const resul = await this.$refs.refsGastos.validate();

            if (resul.valid) {
                this.overlay = true;
                axios.post('/colaborador/editarGastos', {
                    id: this.idItem,
                    data: this.data,
                }).then(res => {
                    this.overlay = false;
        ...
        eliminarGastos(item) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    axios.post('/colaborador/eliminarGastos', {
                        id: item.id
                    }).then(res => {
                        this.overlay = false;
                        if (res.data.status === 'ok') {
